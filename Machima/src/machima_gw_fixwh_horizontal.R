...
infiles_3 <- paste0("output/gromov_wasserstein/", sc_sample,
    "/", chip, "/", hm, "/", bin, "/", chr, "/T.csv")
...
Ts <- lapply(infiles_3, function(x){
    as.matrix(read.csv(x, header=FALSE))
})

# Perform Machima
J <- min(length(unique(label_RNA)), ncol(X_RNAs[[1]]), ncol(X_Epis[[1]]))
out <- Machima(X_RNAs, X_Epis, label=label_RNA, horizontal=TRUE, T=Ts,
    fixW_RNA=TRUE, fixH_RNA=TRUE, fixT=TRUE, J=J, verbose=TRUE)
