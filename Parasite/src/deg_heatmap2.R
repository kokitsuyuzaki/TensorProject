...
for(i in seq(5)){
	...
	tmp <- deg@.Data[[1]]
	pos <- head(rownames(tmp), 0.1*nrow(counts_At))
	pos_at[pos] <- 1
	qtable_at <- cbind(qtable_at, pos_at)
}
...
pos_at[pos] <- 1
qtable_at <- cbind(qtable_at, pos_at)
colnames(qtable_at) <- c("1d", "3d", "7d", "wol", "parasm", "procambium")
...
for(i in seq(5)){
	...
	tmp <- deg@.Data[[1]]
	pos <- head(rownames(tmp), 0.1*nrow(counts_Pj))
	pos_pj[pos] <- 1
	qtable_pj <- cbind(qtable_pj, pos_pj)
}
...
pos_pj[pos] <- 1
qtable_pj <- cbind(qtable_pj, pos_pj)
colnames(qtable_pj) <- c("1d", "3d", "7d", "wol", "parasm", "procambium")
...
colnames(loadings_at) <- paste0("Dim", seq(ncol(loadings_at)))
colnames(loadings_pj) <- paste0("Dim", seq(ncol(loadings_pj)))
